...
它不是一个简单聊天框，而是一个围绕“周末怎么安排”做完整体验闭环的 AI 应用原型。一个核心边界是：**模型可以提出计划，但可执行动作必须重新绑定到服务端本轮检索得到的真实候选记录**；无法通过确定性校验的结构化输出会被拒绝并回退。

## 项目亮点

- **结构化 AI 输出 + 语义约束**：模型输出必须包含 itinerary JSON；服务端不仅检查字段形状，还校验真实日期、时间顺序、步骤重叠、经纬度范围，以及模型选择的场馆/餐厅 ID 是否来自本轮检索候选。
...
- LLM 行程解析、字段校验与语义约束（日期/时间窗/重叠/坐标）
- 模型生成地点 ID 与本轮数据库检索候选的一致性校验
- 预订动作使用数据库 canonical record，而不是信任模型返回的地点名称
- 确定性兜底行程生成
- 密码哈希与密码校验
- 接口速率限制
- LLM 客户端错误处理与重试
